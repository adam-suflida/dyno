...
public class DynoJedisClient implements JedisCommands, BinaryJedisCommands, MultiKeyCommands
{
	...
	public OperationResult<ScanResult<Map.Entry<String, String>>> d_hscan(final String key, final String cursor)
	{
		...
		{
			return connPool.executeWithFailover(new CompressionValueOperation<ScanResult<Map.Entry<String, String>>>(key,
					OpName.HSCAN)
			{
				@Override
				public ScanResult<Map.Entry<String, String>> execute(final Jedis client, final ConnectionContext state)
				{
					return new ScanResult<>(cursor, new ArrayList(CollectionUtils.transform(client.hscan(key, cursor)
									.getResult(),
	...
	private List<OperationResult<ScanResult<String>>> scatterGatherScan(final CursorBasedResult<String> cursor, final int count,
	...
	public Collection<OperationResult<Set<String>>> d_keys(final String pattern)
	{
		...
		Collection<OperationResult<Set<String>>> results = connPool.executeWithRing(new BaseKeyOperation<Set<String>>(pattern,
				OpName.KEYS)
		{
	...
	public OperationResult<List<String>> d_mget(final String... keys)
	{
		...
		{
			return connPool.executeWithFailover(new CompressionValueMultiKeyOperation<List<String>>(Arrays.asList(keys),
					OpName.MGET)
			{
				@Override
				public List<String> execute(final Jedis client, final ConnectionContext state) throws DynoException
	...
	public static class Builder
	{
		...
		private DynoDualWriterClient buildDynoDualWriterClient()
		{
			...
			if (dualWriteHostSupplier == null)
			{
				...
				{
					throw new DynoConnectException(
							"HostSupplier for DualWrite cluster is REQUIRED if you are not " + "using EurekaHostsSupplier " +
									"implementation or using a EurekaClient");
				}
			}
			else
			{
			...
			JedisConnectionFactory connFactory = new JedisConnectionFactory(shadowOPMonitor, sslSocketFactory, sslParameters,
					hostnameVerifier);
				...
				ConnectionPoolMonitor cpMonitor)
		{
			...
			if (hostSupplier == null)
			{
				...
				{
					throw new DynoConnectException(
							"HostSupplier not provided. Cannot initialize EurekaHostsSupplier " + "which requires a " +
									"DiscoveryClient");
				}
				else
				{
		...
		private void setLoadBalancingStrategy(ConnectionPoolConfigurationImpl config)
		{
			if (ConnectionPoolConfiguration.LoadBalancingStrategy.TokenAware == config.getLoadBalancingStrategy())
			{
				...
				{
					String warningMessage = "DynoJedisClient for app=[" + config
							.getName() + "] is configured for local rack affinity " +
